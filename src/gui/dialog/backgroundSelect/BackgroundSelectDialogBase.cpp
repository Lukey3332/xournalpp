...
void BackgroundSelectDialogBase::setSelected(int64_t selected) {
    if (this->selected == selected) {
        return;
    }

    int64_t lastSelected = this->selected;
    if (lastSelected >= 0 && lastSelected < static_cast<int64_t>(elements.size())) {
